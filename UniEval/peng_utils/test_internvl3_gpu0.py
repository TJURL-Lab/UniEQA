...
class TestInternVL3:
    def __init__(self, device=None):
        # 使用本地模型路径
        model_path = '/home/fx/Exp2/test/EmbodiedEval/msjeval/model_zoo/InternVL3-8B'
        ...
        # 优化配置 - 针对benchmark推理速度优化
        self.fast_generation_config = dict(
            max_new_tokens=256,  # 减少输出长度
            do_sample=False,     # 使用贪心搜索，更快
            num_beams=1,         # 单beam搜索
        ...
        if self.is_multi_gpu:
            self.device = 'cuda:0'  # 主GPU使用0，利用最多可用内存
        else:
            self.device = 'cpu'
    ...
    @torch.no_grad()
    def generate_image(self, image_list_list, question_list, fast_mode=True):
        ...
        # 选择生成配置
        gen_config = self.fast_generation_config if fast_mode else self.generation_config
        ...
        try:
            ...
            for contacted_image, question in zip(contacted_images, question_list):
                try:
                    ...
                    # 确保数据类型与模型一致
                    if self.is_multi_gpu:
                        # 多GPU情况下，转换为bfloat16并放到主设备GPU 0
