...
from server_utils.constants import CONTROLLER_HEART_BEAT_EXPIRATION, LOGDIR, WORKER_API_TIMEOUT
...
class Controller:
    ...
    def get_worker_status(self, worker_name: str):
        try:
            r = requests.post(worker_name + "/worker_get_status", timeout=5)#5)
    ...
    def refresh_all_workers(self):
        ...
        for w_name, w_info in old_info.items():
            if not self.register_worker(w_name, w_info.check_heart_beat, None):
                logger.info(f"Remove stale worker: {w_name}")   # return False -> removed
